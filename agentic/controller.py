...
class AgenticTrainingController:
    ...
    def act(
        ...
        budget = torch.full((batch_size,), self.top_r, device=device, dtype=torch.float32)
    ...
    def observe_eval(self, epoch: int, metrics: Dict[str, float], train_stats: Dict[str, float]) -> Dict[str, float | bool | str]:
        ...
        utility = self.gain_ema - self.stop_cost_weight * cost
        stop_ready = (
            self.stop_enabled

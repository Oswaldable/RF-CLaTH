...
run_hmdb() {
  ...
  for bits in $BITS; do
    ...
    CUDA_VISIBLE_DEVICES="$GPU" PYTHONPATH="$PROJECT_ROOT" "$PYTHON_BIN" train.py \
      ...
      --epochs 150 \
      --hash-bits "$bits" \
      --override "project.name=RF-CLaTH-ARF-Memory-Contrastive-HMDB-RePartition" \
...
echo "$(timestamp) | RF-CLaTH ARF memory contrastive HMDB run start, bits=${BITS}, gpu=${GPU}, view=${VIEW_LAMBDA}, batch=${BATCH_LAMBDA}, arf=${ARF_LAMBDA}, pos_topk=${ARF_POSITIVE_TOPK}, hard_neg_w=${ARF_HARD_NEGATIVE_WEIGHT}, log=${log_file}"
run_hmdb >> "$log_file" 2>&1
echo "$(timestamp) | RF-CLaTH ARF memory contrastive HMDB run done"

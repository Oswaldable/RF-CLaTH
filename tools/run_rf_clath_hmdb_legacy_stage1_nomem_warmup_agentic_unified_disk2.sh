...
TAG="${TAG:-legacy_stage1nomem_warm${STAGE1_WARMUP_EPOCHS}_agentic_unified_v1}"
...
run_hmdb() {
  ...
  for bits in $BITS; do
    ...
    CUDA_VISIBLE_DEVICES="$GPU" PYTHONPATH="$PROJECT_ROOT" "$PYTHON_BIN" train.py \
      ...
      --epochs 150 \
      --hash-bits "$bits" \
      --override "project.name=RF-CLaTH-LegacyStage1NoMemWarmup-AgenticUnified-HMDB-RePartition" \
      --override "training.objective=legacy_stage1_warmup_agentic_unified" \
      --override "train.objective=legacy_stage1_warmup_agentic_unified" \
      --override "loss.type=legacy_stage1_warmup_agentic_unified" \
      ...
      --override "agentic_contrastive.stage1_lambda_memory_neighbor=0.0" \
...
echo "$(timestamp) | RF-CLaTH legacy Stage1-no-memory warmup -> Agentic Unified HMDB run start, bits=${BITS}, gpu=${GPU}, warmup_epochs=${STAGE1_WARMUP_EPOCHS}, switch_epoch=${SWITCH_EPOCH}, log=${log_file}"
run_hmdb >> "$log_file" 2>&1
echo "$(timestamp) | RF-CLaTH legacy Stage1-no-memory warmup -> Agentic Unified HMDB run done"

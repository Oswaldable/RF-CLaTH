...
DATASET=s5vh_ucf
OUTPUT_DIR="${OUTPUT_ROOT}/rf_clath_t_sas_ucf_disk2"
...
run_ucf() {
  ...
  for bits in $BITS; do
    echo "$(timestamp) | rf_clath_t_sas_ucf ${bits}-bit start on cuda${GPU}"
    CUDA_VISIBLE_DEVICES="$GPU" PYTHONPATH="$PROJECT_ROOT" "$PYTHON_BIN" train.py \
      ...
      --epochs 150 \
      --hash-bits "$bits" \
      --override "project.name=RF-CLaTH-T-SAS-UCF-RePartition"
  done

  echo "$(timestamp) | rf_clath_t_sas_ucf done"
}

log_file="${LOG_ROOT}/rf_clath_t_sas_ucf_disk2_$(date +%Y%m%d_%H%M%S).queue.log"

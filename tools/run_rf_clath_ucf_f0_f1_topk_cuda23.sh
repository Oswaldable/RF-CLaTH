...
GPU="${1:?usage: $0 <gpu> <f0|f1>}"
CASE_ID="${2:?usage: $0 <gpu> <f0|f1>}"
...
overrides=(
  ...
  "agentic_contrastive.memory_candidate_strategy=hard"
...
case "$CASE_ID" in
  f0)
    case_name=f0_routed_memory_topk64_30ep
    project_name=RF-CLaTH-UCF-F0-RoutedMemoryTopK64-30Ep
    memory_candidate_topk=64
    ;;
  f1)
    case_name=f1_routed_memory_topk128_30ep
    project_name=RF-CLaTH-UCF-F1-RoutedMemoryTopK128-30Ep
    memory_candidate_topk=128
    ;;
  *)
    echo "Unknown case=${CASE_ID}; expected f0 or f1." >&2
    exit 2
    ;;
esac

overrides+=("agentic_contrastive.memory_candidate_topk=${memory_candidate_topk}")
output_dir="${OUTPUT_ROOT}/rf_clath_ucf_f0_f1_topk/${case_name}"
command=(
  "$PYTHON_BIN" train.py
  --config "$CONFIG"
  --dataset "$DATASET"
...
log_file="${LOG_ROOT}/rf_clath_ucf_${CASE_ID}_topk_cuda${GPU}_$(date +%Y%m%d_%H%M%S).queue.log"
echo "$(timestamp) | case=${CASE_ID} name=${case_name} memory_topk=${memory_candidate_topk} bits=${BITS} epochs=${EPOCHS} gpu=${GPU} log=${log_file}"
